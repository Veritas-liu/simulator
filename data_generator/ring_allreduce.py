...
def build_ring_allreduce_flows(n_gpu, base_size, variation):
    """Ring all-reduce: each node sends to next, receives from prev, for n-1 stages."""
    flows = []
    fid = 0
    stage_ids = []

    for stage in range(n_gpu - 1):
        ids = []
...
def main():
    parser = argparse.ArgumentParser(description='Generate ring all-reduce flows.json')
    ...
    if args.n_gpu <= 1:
        raise ValueError('n_gpu must be > 1 for ring all-reduce')
    if args.base_size <= 0:
    ...
    flows = build_ring_allreduce_flows(args.n_gpu, args.base_size, args.variation)
